...
func _on_tree_exiting():
	Logger.debug("Network shutting down")
	pass
	
func start_server() -> void:
	Logger.info("Starting server on port %s" % std.coalesce(Cache.port, DEFAULT_PORT))
	establish_server(std.coalesce(Cache.port, DEFAULT_PORT))
	
func start_client() -> void:
	Logger.info("Starting client connecting to %s:%s" % [std.coalesce(Cache.uri, DEFAULT_URI), std.coalesce(Cache.port, DEFAULT_PORT)])
...
func establish_server(port: int = 5000, max_clients: int = DEFAULT_MAX_NUM_CLIENTS):
	Logger.debug("Establishing server: port=%s, max_clients=%s, trial=%s" % [port, max_clients, OS.has_feature("trial")])
	if OS.has_feature("trial"):
		max_clients = 0
		Logger.trace("Trial version detected, setting max_clients to 0")
	...
	if peer.create_server(port, max_clients) != OK:
		...
		Logger.info("Server created successfully on port %s" % port)
	...
	Logger.trace("Multiplayer peer set for server")
	multiplayer.peer_connected.connect(func(peer_id): 
		Logger.debug("Server received peer_connected signal for peer_id=%s" % peer_id)
		peer_connected.emit(peer_id))
	multiplayer.peer_disconnected.connect(func(peer_id): 
		Logger.debug("Server received peer_disconnected signal for peer_id=%s" % peer_id)
		peer_disconnected.emit(peer_id))
	_defer_signal(func(): server_established.emit())

func establish_client(uri: String = "localhost", port: int = 5000):
	Logger.debug("Establishing client: uri=%s, port=%s, trial=%s" % [uri, port, OS.has_feature("trial")])
	if OS.has_feature("trial"): 
		Logger.trace("Trial version detected, skipping client creation")
		return
	# Create client that connects to a server at uri using specified port.
	if peer.create_client(uri, port) != OK:
		Logger.error("Failed to create client connecting to %s:%s" % [uri, port])
		return
	else:
		Logger.info("Client created successfully, connecting to %s:%s" % [uri, port])
	# The peer object to handle the RPC system (effectively enabling networking
	# when set). 
	multiplayer.set_multiplayer_peer(peer)
	Logger.trace("Multiplayer peer set for client")
	multiplayer.connected_to_server.connect(func(): 
		Logger.debug("Client received connected_to_server signal")
		connected_to_server.emit())
	multiplayer.connection_failed.connect(func(): 
		Logger.debug("Client received connection_failed signal")
		connection_failed.emit())
	multiplayer.server_disconnected.connect(func(): 
		Logger.debug("Client received server_disconnected signal")
		server_disconnected.emit())
	_defer_signal(func(): client_established.emit())
	
func unestablish():
	Logger.info("Unestablishing network connection")
	multiplayer.set_multiplayer_peer(null)
	Logger.debug("Network connection unestablished")
