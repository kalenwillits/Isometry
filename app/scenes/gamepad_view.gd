...
func _show_page(page_num: int) -> void:
	"""Displays the specified page of gamepad bindings"""
	# Clamp page number
...
func _on_reset_all_pressed() -> void:
	...
	dialog.confirmed.connect(func():
		# Only reset gamepad bindings, not keyboard
		for action_name in Keybinds.get_all_actions():
			Keybinds.reset_action_to_default(action_name, "gamepad")
		has_unsaved_changes = true
		_update_save_button_visibility()
...
func _on_back_pressed() -> void:
	...
	if has_unsaved_changes:
		...
		dialog.confirmed.connect(func():
			...
			_update_save_button_visibility()
			_refresh_current_page()
			get_tree().call_group(Group.INTERFACE, "close_gamepad_view")
		)
