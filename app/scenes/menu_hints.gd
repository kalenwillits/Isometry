...
func _build_hints() -> void:
	...
	# Add Accept hint
	if show_accept:
		_add_hint("menu_accept", "Accept")

	# Add Cancel hint
	if show_cancel:
		if get_child_count() > 0:
			_add_spacer()
		_add_hint("menu_cancel", "Cancel")

	# Add Pagination hints
	if show_pagination:
		if get_child_count() > 0:
			_add_spacer()
		_add_hint("menu_previous_page", "Prev")
		_add_spacer()
		_add_hint("menu_next_page", "Next")

func _add_hint(action_name: String, label_text: String) -> void:
	...
	input_icon.icon_size = Vector2i(8, 8)
	input_icon.text_size = 8
